...
function ledgerLabel(kind: "sales" | "purchase", rate: number) {
  return `${kind} ${rate}%`;
}
...
export async function getTallyExportData(startDateStr: string, endDateStr: string) {
  ...
  const salesRows = dbSales.map((s) => {
    ...
    const taxable = parseFloat(s.amount);
  ...
  const purchaseRows = dbPurchases.map((p) => {
    ...
    const taxable = parseFloat(p.amount);
  ...
  const creditNoteRows = dbCreditNotes.map((c) => {
    ...
    const taxable = parseFloat(c.amount);
  ...
  const dbDebitNotes = await db
    .select({
      ...
      gstRate: sql<string>`coalesce(${purchaseReturnItems.gstRate}, ${products.gstRate})`,
    })
  ...
  const debitNoteRows = dbDebitNotes.map((d) => {
    ...
    const taxable = parseFloat(d.amount);
